	...
	<?php 
		if((isset($_REQUEST['saveDraft'])) || (isset($_REQUEST['creChap']))){
			...
			if($fchap_rtn->num_rows == 0){
				//creating chapter id
				$f_lastChap = "SELECT chap_id FROM chapter ORDER BY chap_id DESC LIMIT 1";
				...
				//creating folder for the uploaded new chapter
				$f_sname = "SELECT series_name FROM series WHERE series_id = '$s_id'";
