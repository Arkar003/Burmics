...
<body>
		...
		<div class="container">
			...
			<div class="mb-2 row">
				<div class="text-center">
					<button class="btn btn-primary" type="button" data-bs-toggle="collapse" data-bs-target="#order">Order</button>
				</div>
			</div>
			<div class="mb-3 row collapse" id="order">
				<div class="pw-box mb-2">
					<label class="form-label" for="conPass">Enter your password to confirm the Order.</label>
					<input class="form-control" type="password" name="conPass" id="conPass">
					<i class="bi bi-eye-fill ic admin-ic fs-4" id="show-pass"></i>
				</div>
				<div class="text-center">
	...
	<script type="text/javascript">
		var pwd = document.getElementById("conPass");
	...
	<?php 
		if(isset($_REQUEST['submitForm'])){
			$pass = md5($_REQUEST['conPass']);
			$userID = $_SESSION['uid'];
			$getpass = "SELECT password FROM user WHERE user_id = '$userID'";
			$gp_rtn = mysqli_query($dbconn,$getpass);
			$pword = mysqli_fetch_assoc($gp_rtn);
			if($pass == $pword['password']){
				$cID = getCreatorId($userID);
